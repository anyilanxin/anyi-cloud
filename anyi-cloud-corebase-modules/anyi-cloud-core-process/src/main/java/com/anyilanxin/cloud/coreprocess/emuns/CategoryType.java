...
@Getter
@ConstantType
public enum CategoryType implements ISuperType {
    /**
     * bpmn
     */
    BPMN(1, "bpmn"),
    ...
    CategoryType(int type, String describe) {
        this.type = type;
    ...
    @JsonCreator
    public static CategoryType from(int code) {
        return getByState(code);
    }
     ...
     * @date 2020-09-11 16:02
     */
    public static CategoryType getByState(Integer type) {
        CategoryType[] values = CategoryType.values();
        for (CategoryType value : values) {
            if (value.type == type) {
                return value;
