...
public class CommonCategoryController extends BaseController {
    ...
    @Operation(
            summary = "分类字典表逻辑批量删除",
            tags = {"v1.0.0"},
            description = "批量删除分类字典表")
    @PostMapping(value = "/delete-batch")
    @CacheEvict(value = CoreCommonCacheConstant.ENGINE_DICT_CATEGORY_CACHE, allEntries = true)
    public Result<String> deleteBatch(
            @RequestBody @NotNullSize(message = "待删除分类id不能为空") List<String> categoryIds) {
        service.deleteBatch(categoryIds);
        return ok(I18nUtil.get("Controller.BatchDeleteSuccess"));
    }

    @Operation(
            summary = "根据统一分类id查询分类",
            tags = {"v1.0.0"},
            description = "根据统一分类id查询分类")
    @GetMapping(value = "/select/list/{categoryCommonCode}")
    @Cacheable(
            value = CoreCommonCacheConstant.ENGINE_DICT_CATEGORY_CACHE,
            key = "#categoryCommonCode")
    @Parameter(
            in = ParameterIn.PATH,
            description = "统一分类编码",
            name = "categoryCommonCode",
            required = true)
    ...
    @Operation(
            summary = " 根据统一分类id查询分类(树形)",
            tags = {"v1.0.0"},
            description = " 根据统一分类id查询分类(树形)")
