...
import com.anyilanxin.anyicloud.gateway.utils.DecryptEncryptUtils;
...
public class EncryptGatewayFilterFactory extends AbstractGatewayFilterFactory<EncryptGatewayFilterFactory.Config> {
    ...
    public GatewayFilter apply(Config config) {
        EncryptDecryptGatewayFilter gatewayFilter = new EncryptDecryptGatewayFilter(config);
    ...
    private static class EncryptDecryptGatewayFilter implements GatewayFilter, Ordered {
        ...
        public EncryptDecryptGatewayFilter(Config config) {
            this.config = config;
        }
        ...
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            log.debug("------------EncryptDecryptGatewayFilter------------>filter:{}", "加密过滤器");
            return chain.filter(exchange.mutate().response(DecryptEncryptUtils.encrypt(exchange)).build());
        }
