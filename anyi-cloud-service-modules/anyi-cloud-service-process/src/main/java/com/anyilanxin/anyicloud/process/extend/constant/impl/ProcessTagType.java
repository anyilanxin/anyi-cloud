...
@Getter
@ConstantType
public enum ProcessTagType implements ISuperType {
    /**
     * 正常
     */
    normal("normal", "processTag", "正常"),

    /**
     * 待重新处理
     */
    resubmit("resubmit", "processTag", "待重新提交"),

    /**
     * 完成
     */
    complete("complete", "processTag", "完成");
    ...
    ProcessTagType(String type, String variableKey, String describe) {
        this.type = type;
    ...
    public static boolean isHaveByType(String type) {
        ProcessTagType[] values = ProcessTagType.values();
        for (ProcessTagType value : values) {
     ...
     * @date 2020-09-11 16:02
     */
    public static ProcessTagType getByType(String type) {
        ProcessTagType[] values = ProcessTagType.values();
        for (ProcessTagType value : values) {
            if (value.type.equals(type)) {
                return value;
