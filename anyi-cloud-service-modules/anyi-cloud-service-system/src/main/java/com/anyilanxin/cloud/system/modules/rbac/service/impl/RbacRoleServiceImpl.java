...
import com.anyilanxin.cloud.corecommon.utils.CodeUtil;
import com.anyilanxin.cloud.database.utils.PageUtils;
import com.anyilanxin.cloud.system.core.constant.impl.MenuType;
import com.anyilanxin.cloud.system.modules.rbac.controller.vo.RbacRoleAuthVo;
...
import com.anyilanxin.cloud.system.modules.rbac.controller.vo.RbacRoleVo;
import com.anyilanxin.cloud.system.modules.rbac.entity.RbacMenuEntity;
...
import com.anyilanxin.cloud.system.modules.rbac.service.IRbacRoleMenuService;
import com.anyilanxin.cloud.system.modules.rbac.service.IRbacRoleResourceApiService;
import com.anyilanxin.cloud.system.modules.rbac.service.IRbacRoleService;
import com.anyilanxin.cloud.system.modules.rbac.service.ISyncProcessService;
import com.anyilanxin.cloud.system.modules.rbac.service.dto.*;
import com.anyilanxin.cloud.system.modules.rbac.service.mapstruct.PermissionMenuActionMap;
import com.anyilanxin.cloud.system.modules.rbac.service.mapstruct.RbacRoleAndMenuDtoMap;
import com.anyilanxin.cloud.system.modules.rbac.service.mapstruct.RbacRoleBasicDtoMap;
import com.anyilanxin.cloud.system.modules.rbac.service.mapstruct.RbacRoleCopyMap;
...
import static com.anyilanxin.cloud.corecommon.constant.SysBaseConstant.SUPER_ROLE;
...
public class RbacRoleServiceImpl extends ServiceImpl<RbacRoleMapper, RbacRoleEntity> implements IRbacRoleService {
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class, Error.class}, readOnly = true)
    public AnYiPageResult<RbacRolePageDto> pageByModel(RbacRolePageQuery vo) throws RuntimeException {
        return PageUtils.toPageData(mapper.pageByModel(PageUtils.getPage(vo), vo, AuthConstant.SUPER_ROLE));
    }
    ...
    public void deleteById(String roleId) throws RuntimeException {
        ...
        // 删除用户关联
        rbacRoleUserMapper.physicalDeleteRoleUserBatchIds(roleIds);
        // 同步流程引擎
    ...
    public void updateStatus(String roleId, Integer status) throws RuntimeException {
        ...
        if (SUPER_ROLE.equals(byId.getRoleCode())) {
            throw new AnYiResponseException(AnYiResultStatus.VERIFICATION_FAILED, "超级管理员角色状态不可操作，永远启用");
        }
    ...
    public List<RbacRoleBasicDto> getEffectiveRoles() throws RuntimeException {
        ...
        list.forEach(v -> {
            // 判断是否为超级管理员角色
            RbacRoleBasicDto rbacRoleBasicDto = basicDtoMap.dToE(v);
    ...
    public List<RbacProcessCommonDto> selectProcessDesignerByIds(List<String> ids) {
        List<RbacRoleEntity> rbacRoleEntities = this.listByIds(ids);
        if (CollUtil.isEmpty(rbacRoleEntities)) {
            return Collections.emptyList();
        }
        return rbacRoleEntities.stream().map(v -> {
            RbacProcessCommonDto dto = new RbacProcessCommonDto();
            dto.setIndex(v.getRoleId());
    ...
    @Override
    public List<RoleInfo> getRoleListByIds(List<String> roleIds) {
        ...
        List<RoleInfo> result = new ArrayList<>(rbacRoleEntities.size());
        rbacRoleEntities.forEach(v -> {
            RoleInfo roleInfo = basicDtoMap.dToV(v);
            // 判断是否为超级管理员角色
            if (AuthConstant.SUPER_ROLE.equals(roleInfo.getRoleCode())) {
                roleInfo.setSuperRole(true);
            }
            result.add(roleInfo);
        });
        return result;
