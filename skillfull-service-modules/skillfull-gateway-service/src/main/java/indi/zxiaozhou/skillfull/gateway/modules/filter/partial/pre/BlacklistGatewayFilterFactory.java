...
public class BlacklistGatewayFilterFactory extends AbstractGatewayFilterFactory<BlacklistGatewayFilterFactory.Config> {
    ...
    private static class VerifySignGatewayFilter implements GatewayFilter, Ordered {
        ...
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            log.debug("------------VerifySignGatewayFilter------------>filter:{}", "黑名单过滤器");
            // 检测是否为黑名单
            GatewayCommonUtils.CheckUrlInfo haveUrl = GatewayCommonUtils.isHaveUrl(exchange, config.getSpecialUrl());
            if (haveUrl.isResult()) {
                return GatewayCommonUtils.responseWrite(exchange, HttpStatus.FORBIDDEN, Status.ACCESS_DENIED, Status.ACCESS_DENIED.getMessage(), null);
            }
        ...
        @Override
        public int getOrder() {
            return FilterCustomPreType.BLACKLIST.getOrder();
        }
