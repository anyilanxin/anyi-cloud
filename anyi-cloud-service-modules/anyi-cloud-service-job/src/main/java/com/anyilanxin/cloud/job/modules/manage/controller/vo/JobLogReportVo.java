...
public class JobLogReportVo implements Serializable {
    @Serial
    private static final long serialVersionUID = -75386932091542994L;
    ...
    @Schema(name = "runningCount", title = "运行中-日志数量", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlankOrNull(message = "运行中-日志数量不能为空")
    private Integer runningCount;

    @Schema(name = "sucCount", title = "执行成功-日志数量", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlankOrNull(message = "执行成功-日志数量不能为空")
    private Integer sucCount;

    @Schema(name = "failCount", title = "执行失败-日志数量", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlankOrNull(message = "执行失败-日志数量不能为空")
