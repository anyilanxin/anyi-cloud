...
// +----------------------------------------------------------------------
package indi.zxiaozhou.skillfull.message.core.constant.impl;
...
@Getter
@ConstantType
public enum MsgTemplateType implements ISuperType {
    /**
     * 微信模板消息
     */
    WX(1, "微信模板消息"),
    ...
    MsgTemplateType(int type, String describe) {
        this.type = type;
    ...
    public static boolean isHaveByType(int type) {
        MsgTemplateType[] values = MsgTemplateType.values();
        for (MsgTemplateType value : values) {
            if (value.type == type) {
                return true;
            }
        }
