...
package com.anyilanxin.anyicloud.storageadapter.utils;

import com.anyilanxin.anyicloud.storageadapter.model.AnYiMultipartFile;
...
public class StorageFileUtil {
    ...
    public static MultipartFile fileToMultipartFile(File file, String requestFileName) {
        ...
        try {
            if (StringUtils.isBlank(requestFileName)) {
                requestFileName = "file";
            }
            return new AnYiMultipartFile(requestFileName, file, contentType);
        } catch (IOException e) {
    ...
    public static MultipartFile fileToMultipartFile(File file) {
        String contentType = new MimetypesFileTypeMap().getContentType(file);
        try {
            return new AnYiMultipartFile(file, contentType);
        } catch (IOException e) {
            throw new RuntimeException("文件转换为MultipartFile异常:" + e.getMessage());
        }
    }
