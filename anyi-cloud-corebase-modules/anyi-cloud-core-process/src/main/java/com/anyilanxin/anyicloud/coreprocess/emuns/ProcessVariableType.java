...
import com.anyilanxin.anyicloud.coreprocess.model.AnYiZeebeProcessGrobalInfoModel;
...
@Getter
@ConstantType
public enum ProcessVariableType implements ISuperType {
    /**
     * 关联流程实例id
     */
    ASSOCIATE_PROCESS_INSTANCE_ID("processAssociateInstanceId", "processAssociateInstanceId", "关联流程实例id(泳池消息启动时注入)"),
    /**
     * 激活流程实例id
     */
    ACTIVATION_PROCESS_INSTANCE_ID("processActivationInstanceId", "processActivationInstanceId", "激活流程实例id(泳池消息启动时注入)"),
    /**
     * 流程业务id
     */
    PROCESS_BUSINESS_KEY("businessKey", "businessKey", "流程业务id"),
    /**
     * 流程标题
     */
    PROCESS_TITLE("processStartTitle", "processStartTitle", "流程标题"),
    /**
     * 模型名称
     */
    MODEL_NAME("diagramName", "diagramName", "模型名称"),

    /**
     * 发起时间
     */
    INITIATE_TIME("initiateTime", "initiateTime", "发起时间"),
    /**
     * 流程启动人
     */
    INITIATE_USER("initiateUser", "initiateUser", "流程提交人(系统变量)"),

    /**
     * 流程类型
     */
    PROCESS_CATEGORY("processCategory", "processCategory", "流程类型"),

    /**
     * 流程类型名称
     */
    PROCESS_CATEGORY_NAME("processCategoryName", "processCategoryName", "流程类型名称"),
    /**
     * 流程提交人所有信息字符串变量
     */
    INITIATE_USER_INFO_STR("initiateUserStr", "initiateUserStr", "流程提交人所有信息字符串变量"),
     ...
     * 流程表单数据
     */
    PROCESS_FORM("processForm", "processForm", "流程表单数据"),

    /**
     * 流程表单数据
     */
    PROCESS_ATTACHMENT("processAttachment", "processAttachment", "流程附件数据"),
    /**
     * 任务类型
     */
    ...
    /**
     * anyi zeebe引擎提交审批时全局变量，具体参考${@link AnYiZeebeProcessGrobalInfoModel}
     */
    ANYI_ZEEBE_PROCESS_GROBAL_INFO("anYiZeebeProcessGrobalInfo", "anYiZeebeProcessGrobalInfo", "zeebe提交申请基础参数，具体参考类型AnYiZeebeProcessGrobalInfoModel"),
    ...
    ProcessVariableType(String type, String variableKey, String describe) {
        this.type = type;
    ...
    public static boolean isHaveByType(String type) {
        ProcessVariableType[] values = ProcessVariableType.values();
        for (ProcessVariableType value : values) {
     ...
     * @date 2020-09-11 16:02
     */
    public static ProcessVariableType getByType(String type) {
        ProcessVariableType[] values = ProcessVariableType.values();
        for (ProcessVariableType value : values) {
            if (value.type.equals(type)) {
                return value;
