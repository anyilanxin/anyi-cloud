...
// +----------------------------------------------------------------------
package indi.zxiaozhou.skillfull.corecommon.base.service;
...
public interface ICoreCommonService {
    ...
    static void loadConstantDict(boolean force, RedisTemplate<String, Object> redisTemplate, String serviceName) {
        if (!force) {
            Object redisLockValue = redisTemplate.opsForValue().get(ICoreCommonService.REDIS_LOCK_KEY + serviceName);
        ...
        if (CollectionUtil.isNotEmpty(constantDict)) {
            constantDict.forEach((k, v) -> redisTemplate.opsForValue().set(CoreCommonCacheConstant.ENGINE_CONSTANT_DICT_CACHE + k, v));
        }
        redisTemplate.opsForValue().set(ICoreCommonService.REDIS_LOCK_KEY + serviceName, true, LOCK_EXPIRES, TimeUnit.SECONDS);
    }
}
