...
package com.anyilanxin.cloud.coremvc.aspect;
...
import com.anyilanxin.cloud.coremvc.aspect.model.OperateModel;
...
public class LogAspect {
    ...
    @Pointcut("@annotation(com.anyilanxin.cloud.corecommon.annotation.AutoLog)")
    ...
    private void saveSysLog(ProceedingJoinPoint joinPoint, Object result) {
        ...
        if (ServletUtils.isAjaxRequest(request)) {
            if (CollUtil.isNotEmpty(request.getParameterMap())) {
                model.setRequestParam(AnYiCoreCommonUtils.objectToJsonStr(request.getParameterMap()));
            }
        }
        if (result instanceof Exception) {
            ...
            model.setRequestResult(Objects.nonNull(result) ? AnYiCoreCommonUtils.objectToJsonStr(result) : "");
        }
    }
            ...
            if (args != null && parameterNames.length == args.length) {
                for (int i = 0; i < len; i++) {
                    ...
                    if (!isFilterObject(objectParam)) {
                        try {
                            jsonObject.put(parameterNames[i], JSON.toJSON(objectParam));
                        } catch (Exception e) {
