...
package com.anyilanxin.anyicloud.process.core.handler;
...
public class GlobalExceptionHandler extends AnYiBaseController {
     ...
     * 处理流程引擎错误
     *
     * @param e ${@link ProcessEngineException}
     * @return AnYiResult<?> ${@link AnYiResult <?>}
     * @author zxh
     * @date 2020-08-27 15:28
     */
    @ExceptionHandler(ProcessEngineException.class)
    public AnYiResult<?> handleProcessEngineException(ProcessEngineException e) {
        e.printStackTrace();
        log.error("------------GlobalExceptionHandler------处理流程引擎错误------>handleBadSqlGrammarException:\n{}", e.getLocalizedMessage());
        Throwable cause = e.getCause();
        while (Objects.nonNull(cause) && Objects.nonNull(cause.getCause())) {
            cause = cause.getCause();
        }
        if (cause instanceof AnYiResponseException responseException) {
            AnYiResult<Object> result = responseException.getResult();
            return fail(result.getCode(), "流程引擎处理错误:" + result.getMessage());
        }
        return fail("流程引擎处理错误:" + e.getMessage());
    }
}
