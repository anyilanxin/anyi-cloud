...
package com.anyilanxin.cloud.oauth2resourcewebflux.config;
...
/**
 * @author zxh
 * @date 2023-09-23 01:59
 * @since 1.0.0
 */
public class AnYiCustomServerBearerTokenAuthenticationConverter extends ServerBearerTokenAuthenticationConverter {
    ...
    public AnYiCustomServerBearerTokenAuthenticationConverter() {
        super();
    }
    ...
    private String resolveAccessTokenFromRequest(ServerHttpRequest request) {
        ...
        if (CollectionUtils.isEmpty(parameterTokens)) {
            parameterTokens = request.getQueryParams().get(ACCESS_TOKEN_QUERY_NAME_DEFAULT);
            if (CollectionUtils.isEmpty(parameterTokens)) {
                return null;
            }
        }
    ...
    private String resolveFromAuthorizationHeader(HttpHeaders headers) {
        ...
        if (!StringUtils.startsWithIgnoreCase(authorization, "bearer")) {
            authorization = headers.getFirst(HttpHeaders.AUTHORIZATION);
            if (!StringUtils.startsWithIgnoreCase(authorization, "bearer")) {
                return null;
            }
        }
