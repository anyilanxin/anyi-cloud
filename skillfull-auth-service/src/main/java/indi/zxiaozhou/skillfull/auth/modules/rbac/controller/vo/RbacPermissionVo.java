...
import indi.zxiaozhou.skillfull.auth.core.constant.impl.IconType;
import indi.zxiaozhou.skillfull.auth.core.constant.impl.PermissionStatus;
...
public class RbacPermissionVo implements Serializable {
    ...
    @Schema(name = "permissionType", title = "权限类型(0:目录; 1:菜单:2:按钮),来源于常量字典:PermissionType", required = true)
    @NotBlankOrNull(message = "权限类型(0:目录; 1:菜单:2:按钮),来源于常量字典:PermissionType不能为空")
    @NotInEnum(message = "类型只能是0、1、2", enumClass = PermissionType.class)
    ...
    @NotBlankOrNull(message = "权限状态不能为空")
    @NotInEnum(message = "权限状态只能是0、1", enumClass = PermissionStatus.class)
    ...
    @NotInEnum(message = "图标类型只能是0、1", enumClass = IconType.class)
...
//    @NotInEnum(message = "按钮校验方式类型不存在", enumClass = ActionType.class)
//    private String checkStrategy;

    @Schema(name = "actionType", title = "按钮权限类型:1-前端型,2-后端型,3-前后端型")
    private Integer actionType;

    @Schema(name = "actionUri", title = "后端uri(相对地址,actionType为true时必填)")
