...
public class DefinitionManageController extends BaseController {
    ...
    public Result<PageDto<ProcessDefinitionPageDto>> selectPageDefinition(
            @RequestBody @Valid ProcessDefinitionPageVo pageVo) {
    ...
    public Result<String> deleteDeploymentById(
            @PathVariable @PathNotBlankOrNull(message = "部署id不能为空") String deploymentId) {
    ...
    @Parameter(
            in = ParameterIn.PATH,
            description = "流程定义id或流程定义key",
            name = "processKeywordId",
            required = true)
    ...
    public Result<String> processDefinitionUpdateState(
            @RequestBody @Valid UpdateProcessDefinitionStateVo vo) {
