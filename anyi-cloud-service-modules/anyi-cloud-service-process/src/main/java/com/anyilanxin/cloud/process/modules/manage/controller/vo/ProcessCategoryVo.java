...
public class ProcessCategoryVo implements Serializable {
    ...
    @Schema(name = "categoryCode", title = "类别编码(唯一)", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlankOrNull(message = "类别编码(唯一)不能为空")
    private String categoryCode;

    @Schema(name = "categoryName", title = "类别名称", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlankOrNull(message = "类别名称不能为空")
    private String categoryName;

    @Schema(name = "categoryState", title = "类别状态:0-禁用,1-启用。默认0", requiredMode = Schema.RequiredMode.REQUIRED)
