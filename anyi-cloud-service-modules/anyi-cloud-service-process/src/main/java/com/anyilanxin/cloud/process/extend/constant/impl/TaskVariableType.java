...
@Getter
@ConstantType
public enum TaskVariableType implements ISuperType {
     ...
     * 任务审批用户信息
     */
    NR_OF_INSTANCE_USER_INFO("nrOfInstanceUserInfo", "nrOfInstanceUserInfo", "任务审批用户信息,json字符串，序列化后为:AnYiProcessUserModel"),
     ...
     * 委托任务审批用户信息
     */
    NR_OF_INSTANCE_DELEGATE_USER_INFO("nrOfInstanceDelegateUserInfo", "nrOfInstanceDelegateUserInfo", "委托任务审批用户信息,json字符串，序列化后为:AnYiProcessUserModel"),
    ...
    TaskVariableType(String type, String variableKey, String describe) {
        this.type = type;
    ...
    public static boolean isHaveByType(String type) {
        TaskVariableType[] values = TaskVariableType.values();
        for (TaskVariableType value : values) {
     ...
     * @date 2020-09-11 16:02
     */
    public static TaskVariableType getByType(String type) {
        TaskVariableType[] values = TaskVariableType.values();
        for (TaskVariableType value : values) {
            if (value.type.equals(type)) {
                return value;
