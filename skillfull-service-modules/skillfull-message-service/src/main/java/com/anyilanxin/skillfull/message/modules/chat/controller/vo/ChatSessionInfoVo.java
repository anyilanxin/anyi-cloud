...
@NoArgsConstructor
@Schema
public class ChatSessionInfoVo implements Serializable {
    private static final long serialVersionUID = -82972886606264619L;
    ...
    @Schema(name = "noReadMsgNum", title = "未读消息数量", required = true)
    @NotBlankOrNull(message = "未读消息数量不能为空")
    private Integer noReadMsgNum;

    @Schema(name = "currentReceiveMsgTime", title = "最近接收消息时间", type = "string", example = "2020-11-12 11:23:59", required = true)
    @NotBlankOrNull(message = "最近接收消息时间不能为空")
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss", timezone = TIME_ZONE_GMT8)
    private LocalDateTime currentReceiveMsgTime;

    @Schema(name = "readMsgNum", title = "已读消息数量", required = true)
    @NotBlankOrNull(message = "已读消息数量不能为空")
    ...
    @Schema(name = "receiverId", title = "接收id,单例为用户id,聊为群id", required = true)
    @NotBlankOrNull(message = "接收id,单例为用户id,聊为群id不能为空")
