...
import com.anyilanxin.skillfull.process.modules.base.mapper.DesignModelMapper;
...
import com.anyilanxin.skillfull.process.modules.base.service.mapstruct.DesignModelToHistoryCopyMap;
...
public class DefinitionManageServiceImpl implements IDefinitionManageService {
    ...
    public ProcessInfoDto getProcessInfo(ProcessInfoVo vo) {
        // 获取流程定义信息
        if (StringUtils.isBlank(vo.getProcessDefinitionId())
                && StringUtils.isBlank(vo.getProcessDefinitionKey())) {
        ...
        if (Objects.nonNull(deployment)) {
            ...
            processInfoDto.setDeploymentTime(
                    CoreCommonDateUtils.toLocalDateTime(deployment.getDeploymentTime()));
        }
        // 补全类别信息
        ProcessCategoryDto processCategoryDto =
                categoryService.selectByCode(processInfoDto.getCategory());
    ...
    @Transactional(
            ...
            readOnly = true)
    public PageDto<ProcessDefinitionPageDto> selectPageDefinition(ProcessDefinitionPageVo pageVo)
        ...
        List<ProcessDefinition> processDefinitions =
                leave.listPage(pageVo.getCurrent(), pageVo.getSize());
        // 转换为自定义类型
        ...
        if (CollUtil.isNotEmpty(processDefinitionPageDtoList)) {
                ...
                processInstanceList.forEach(
                        v -> {
                            List<HistoricProcessInstance> processInstances =
                                    processInstanceMap.get(v.getProcessDefinitionId());
            ...
            List<ReDeploymentEntity> reDeploymentEntities =
                    deploymentMapper.selectBatchIds(deploymentIds);
            ...
            // 获取类别信息
            List<ProcessCategoryDto> processCategoryList =
                    categoryService.selectListByCodes(categoryCodes);
            ...
            // 补全信息
            processDefinitionPageDtoList.forEach(
                    v -> {
                        List<HistoricProcessInstance> processInstances =
                                processInstanceMap.get(v.getProcessDefinitionId());
    ...
    public void createDeployment(DeploymentVo vo) throws RuntimeException {
        ...
        DeploymentBuilder deploymentBuilder =
                repositoryService.createDeployment().name(vo.getDeploymentName());
        deploymentBuilder.addInputStream(
                vo.getDeploymentName() + CommonProcessConstant.MODEL_RESOURCE_SUFFIX,
                Base64FileUtils.base64ToInputStream(designModelEntity.getDiagramData()));
        if (StringUtils.isNotBlank(UserContextUtils.getCurrentTenantId())) {
            deploymentBuilder.tenantId(UserContextUtils.getCurrentTenantId());
        }
        if (Objects.nonNull(vo.getActivateProcessDate())) {
            deploymentBuilder.activateProcessDefinitionsOn(
                    CoreCommonDateUtils.toDateTime(vo.getActivateProcessDate()));
        }
    ...
    public void historyDeployment(DeploymentHistoryVo vo) {
        DesignModelHistoryEntity designModelHistoryEntity =
                modelHistoryMapper.selectById(vo.getHistoryModelId());
    ...
    @Override
    public void processDefinitionUpdateState(UpdateProcessDefinitionStateVo vo)
            throws RuntimeException {
        if (vo.getState()) {
            repositoryService.activateProcessDefinitionById(
                    vo.getProcessDefinitionId(), vo.getProcessInstances(), new Date());
        } else {
    ...
    public DeploymentDetailDto getDeploymentDetail(String processKeywordId) {
        ProcessDefinition processDefinition =
                repositoryService
                        .createProcessDefinitionQuery()
                        .processDefinitionKey(processKeywordId)
                        .latestVersion()
                        .singleResult();
        if (Objects.isNull(processDefinition)) {
            processDefinition =
                    repositoryService
                            .createProcessDefinitionQuery()
                            .processDefinitionId(processKeywordId)
                            .singleResult();
        }
        if (Objects.isNull(processDefinition)) {
        ...
        if (deploymentDetailDto.isHasStartFormKey()) {
            String startFormKey =
                    formService.getStartFormKey(deploymentDetailDto.getProcessDefinitionId());
