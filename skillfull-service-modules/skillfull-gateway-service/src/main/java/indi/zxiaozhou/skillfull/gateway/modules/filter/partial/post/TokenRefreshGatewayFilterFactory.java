...
// +----------------------------------------------------------------------
package indi.zxiaozhou.skillfull.gateway.modules.filter.partial.post;

import com.alibaba.fastjson.JSONObject;
import indi.zxiaozhou.skillfull.corecommon.base.model.stream.router.RouteMetaSpecialUrlModel;
import indi.zxiaozhou.skillfull.gateway.core.constant.typeimpl.FilterCustomPostType;
...
public class TokenRefreshGatewayFilterFactory extends AbstractGatewayFilterFactory<TokenRefreshGatewayFilterFactory.Config> {
    ...
    public GatewayFilter apply(Config config) {
        TokenRefreshGatewayFilter gatewayFilter = new TokenRefreshGatewayFilter(config);
        gatewayFilter.setFactory(this);
        return gatewayFilter;
    }


    private static class TokenRefreshGatewayFilter implements GatewayFilter, Ordered {

        private final Config config;

        private GatewayFilterFactory<Config> gatewayFilterFactory;

        public TokenRefreshGatewayFilter(Config config) {
            this.config = config;
        }

        @Override
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            log.debug("------------TokenRefreshGatewayFilter------------>filter:{}", "token刷新过器");
            // 继续运行
        ...
        @Override
        public int getOrder() {
            return FilterCustomPostType.TOKEN_REFRESH.getOrder();
        }
