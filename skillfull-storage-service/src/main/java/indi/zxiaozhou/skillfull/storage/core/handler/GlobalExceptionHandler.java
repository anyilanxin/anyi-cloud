...
public class GlobalExceptionHandler extends BaseController {
     ...
     * 处理上传文件为空
     *
     * @param e ${@link DecodingException} 处理异常
     * @return Mono<Result < String>> ${@link Mono<Result<String>>} 响应前端
     * @author zxiaozhou
     * @date 2020-08-27 15:17
     */
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public Mono<Result<String>> handlerDecodingException(DecodingException e) {
        log.error("------------GlobalExceptionHandler------处理上传文件为空------>handlerDecodingException:{}", e.getMessage());
        return fail(Status.VERIFICATION_FAILED, "请求中未发现上传文件信息,请检查");
    }
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public Mono<Result<String>> handlerDataBufferLimitException(DataBufferLimitException e) {
        String localizedMessage = e.getLocalizedMessage();
        log.error("------------GlobalExceptionHandler------处理上传文件大小超过限制------>handlerDataBufferLimitException:\n{}", e.getMessage());
        return fail("当前上传文件大小超过系统限制:" + localizedMessage);
    }
}
