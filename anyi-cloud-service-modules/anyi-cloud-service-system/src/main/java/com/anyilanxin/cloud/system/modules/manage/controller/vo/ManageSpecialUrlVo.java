...
package com.anyilanxin.cloud.system.modules.manage.controller.vo;
...
public class ManageSpecialUrlVo implements Serializable {
    ...
    @Schema(name = "urlName", title = "接口名称", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(name = "specialUrlType", title = "特殊url类型:1-白名单(放行url),2-黑名单(只处理url)", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(name = "specialStatus", title = "特殊地址状态:0-禁用,1-启用，默认0", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlankOrNull(message = "特殊地址状态不能为空")
    private Integer specialStatus;

    @Schema(name = "limitMethod", title = "限制请求方法：0-不限制,1-限制", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(name = "url", title = "地址,服务地址或http地址", requiredMode = Schema.RequiredMode.REQUIRED)
