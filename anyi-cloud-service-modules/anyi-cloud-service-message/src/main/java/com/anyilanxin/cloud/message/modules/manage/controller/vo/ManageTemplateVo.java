...
import com.anyilanxin.cloud.corecommon.validation.annotation.NotInEnum;
import com.anyilanxin.cloud.messageadapter.constant.enums.MsgTemplateCommonChannelType;
...
public class ManageTemplateVo implements Serializable {
    ...
    @Schema(name = "templateStatus", title = "模板状态:0-禁用,1-启用", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(name = "sendMaxNum", title = "最大重试次数,默认0-不重试", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlankOrNull(message = "最大重试次数,默认0-不重试不能为空")
    private Integer sendMaxNum;

    @Schema(name = "templateType", title = "模板类型:1-微信模板,2-短信,3-邮件", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(name = "limitSend", title = "是否限制发送次数：0-不限制,1-限制。默认0", requiredMode = Schema.RequiredMode.REQUIRED)
