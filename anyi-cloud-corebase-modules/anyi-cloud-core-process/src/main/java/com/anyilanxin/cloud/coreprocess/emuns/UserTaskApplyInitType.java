...
/**
 * 申请人用户任务初始类型，一般用于激活申请人任务设置类型类处理用户任务初始状态
 *
 * @author zxh
...
@Getter
@ConstantType
public enum UserTaskApplyInitType implements ISuperType {
    /**
     * 提交审批
     */
    TASK_HANDLE_APPLY("taskHandleApply", "提交审批"),

    /**
     * 打回
     */
    TASK_HANDLE_BACK_TO("taskHandleBackTo", "打回"),

    /**
     * 驳回
     */
    TASK_HANDLE_DISMISS("taskHandleDismiss", "驳回"),
    ...
    UserTaskApplyInitType(String type, String describe) {
        this.type = type;
    ...
    public static boolean isHaveByType(String type) {
        UserTaskApplyInitType[] values = UserTaskApplyInitType.values();
        for (UserTaskApplyInitType taskType : values) {
     ...
     * @date 2020-09-11 16:02
     */
    public static UserTaskApplyInitType getByType(String type) {
        UserTaskApplyInitType[] values = UserTaskApplyInitType.values();
        for (UserTaskApplyInitType taskType : values) {
            if (Objects.equals(taskType.type, type)) {
                return taskType;
