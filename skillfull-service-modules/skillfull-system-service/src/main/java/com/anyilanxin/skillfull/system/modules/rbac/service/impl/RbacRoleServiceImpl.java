...
public class RbacRoleServiceImpl extends ServiceImpl<RbacRoleMapper, RbacRoleEntity>
        implements IRbacRoleService {
    ...
    */
    ...
    synchronized String generateCode(String oldParentId, String newParentId, boolean isUpdate) {
        ...
        if (StringUtils.isBlank(newParentId)) {
            ...
            lambdaQueryWrapper
                    .and(
                            v ->
                                    v.isNull(RbacRoleEntity::getParentRoleId)
                                            .or()
                                            .eq(RbacRoleEntity::getParentRoleId, ""))
            ...
            LambdaQueryWrapper<RbacRoleEntity> lambdaQueryWrapper = new LambdaQueryWrapper<>();
            lambdaQueryWrapper
                    .like(RbacRoleEntity::getRoleSysCode, byId.getRoleSysCode() + "____")
                    .orderByDesc(RbacRoleEntity::getRoleSysCode)
                    .last("LIMIT 1");
            RbacRoleEntity one = this.getOne(lambdaQueryWrapper);
            code =
    ...
    public Set<RbacRoleMenuButtonDto> getMenuActions(String roleId) throws RuntimeException {
        ...
        if (SUPER_ROLE.equals(role.getRoleCode())) {
            ...
            if (CollUtil.isNotEmpty(rbacPermissionEntities)) {
                List<RbacRoleMenuDto> superRbacRoleMenuDtos =
                        rbacRoleAndMenuDtoMap.bToA(rbacPermissionEntities);
            ...
            List<RbacRoleMenuDto> rbacRolePermission =
                    roleMenuMapper.selectMenuAntButton(role.getRoleId());
        ...
        if (CollUtil.isNotEmpty(rbacRolePermissionDtos)) {
            ...
            rbacRolePermissionDtos.forEach(
                    v -> {
    ...
    public List<RbacRoleBasicDto> getListByCodes(List<String> roleCodes) {
        LambdaQueryWrapper<RbacRoleEntity> lambdaQueryWrapper =
                Wrappers.<RbacRoleEntity>lambdaQuery().in(RbacRoleEntity::getRoleCode, roleCodes);
    ...
    public List<RbacRoleBasicDto> getRoleListByIds(List<String> roleIds) {
        LambdaQueryWrapper<RbacRoleEntity> lambdaQueryWrapper =
                Wrappers.<RbacRoleEntity>lambdaQuery().in(RbacRoleEntity::getRoleId, roleIds);
        List<RbacRoleEntity> rbacRoleEntities = this.list(lambdaQueryWrapper);
