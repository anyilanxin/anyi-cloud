...
package com.anyilanxin.cloud.corecommon.feign;

import com.anyilanxin.cloud.corecommon.constant.SysBaseConstant;
import com.anyilanxin.cloud.corecommon.feign.strategy.header.ContextHeaderStrategy;
import com.anyilanxin.cloud.corecommon.feign.strategy.safety.ContextSafetyStrategy;
...
public class FeignInterceptorCommon implements RequestInterceptor {
    ...
    public void apply(RequestTemplate template) {
        ...
        // 1. 处理请求头设置，调用特定的
        String headerStrategy = strategies.get(SysBaseConstant.ANYILANXIN_FEIGN_HEADER_KEY);
        if (StringUtils.isNotBlank(headerStrategy)) {
            contextHeaderStrategy.setHeader(headerStrategy, template);
        }
        // 1. 调用一次默认的
        contextHeaderStrategy.setHeader(SysBaseConstant.ANYILANXIN_FEIGN_DEFAULT, template);
        // 2. 调用内部client的
        contextHeaderStrategy.setHeader(SysBaseConstant.ANYILANXIN_CLIENT_SET_TOKEN, template);
        // 处理数据安全
        String safetyStrategy = strategies.get(SysBaseConstant.ANYILANXIN_FEIGN_SAFETY_KEY);
