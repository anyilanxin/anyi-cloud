...
public class RbacUserController extends BaseController {
  ...
  public Result<String> updateState(
      @RequestParam(required = false) @NotBlank(message = "用户id不能为空") String userId,
  ...
  public Result<String> deleteBatchByIds(
      @RequestBody @NotNullSize(message = "待删除用户id不能为空") List<String> userIds) {
  ...
  public Result<RbacUserDto> getById(
      @RequestParam(required = false) @PathNotBlankOrNull(message = "用户id不能为空") String userId,
  ...
  @Operation(
      summary = "分页查询可关联的用户信息",
      tags = {"v1.0.0"},
      description = "分页查询可关联的用户信息")
  @PostMapping(value = "/select/enable-user-page")
  public Result<PageDto<RbacUserPageDto>> selectEnableUserPage(
      @RequestBody @Valid RbacEnalbeUserPageVo vo) {
