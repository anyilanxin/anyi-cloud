...
package com.anyilanxin.anyicloud.messageadapter.constant.enums;
...
@Getter
@ConstantType
public enum SocketMstContentType implements ISuperType {
    /**
     * 刷新数据
     */
    REFRESH_DATA(0, "刷新数据"),
    ...
    SocketMstContentType(int type, String describe) {
        this.type = type;
     ...
     */
    public static SocketMstContentType getByType(int type) {
        SocketMstContentType[] values = SocketMstContentType.values();
        for (SocketMstContentType value : values) {
            if (value.type == type) {
                return value;
            }
        }
