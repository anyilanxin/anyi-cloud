...
@Getter
@ConstantType
public enum IdentityType implements ISuperType {
    /**
     * 办理人
     */
    ASSIGNEE(1, "assignee", "办理人"),

    /**
     * 候选信息
     */
    CANDIDATE(2, "candidate", "候选信息");
    ...
    IdentityType(int type, String typeName, String describe) {
        this.type = type;
     ...
     * @date 2020-09-11 16:02
     */
    public static IdentityType getByType(int type) {
        IdentityType[] values = IdentityType.values();
        for (IdentityType value : values) {
            if (value.type == type) {
                return value;
     ...
     * @return LbType ${@link IdentityType} 匹配的类型
     * @author zxh
     * @date 2020-09-28 09:32
     */
    public static IdentityType getByTypeName(String typeName) {
        IdentityType[] values = IdentityType.values();
        for (IdentityType value : values) {
