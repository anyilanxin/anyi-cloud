...
package com.anyilanxin.cloud.coremvc.config.dictload;
...
import static com.anyilanxin.cloud.corecommon.constant.SysBaseConstant.BOOT_BASE_SCAN_PACKAGE;
 ...
 * @since 1.0.0
 */
public interface ICoreWebmvcService {
    ...
    static void loadConstantDict(boolean force, RedisTemplate<String, Object> redisTemplate, String serviceName) {
        if (!force) {
            Object redisLockValue = redisTemplate.opsForValue().get(ICoreWebmvcService.REDIS_LOCK_KEY + serviceName);
            if (Objects.nonNull(redisLockValue)) {
                return;
            }
        }
        Map<String, List<ConstantDictModel>> constantDict = AnYiCoreCommonUtils.createOrGetConstantDict(serviceName, BOOT_BASE_SCAN_PACKAGE);
        ...
        redisTemplate.opsForValue().set(ICoreWebmvcService.REDIS_LOCK_KEY + serviceName, true, LOCK_EXPIRES, TimeUnit.SECONDS);
    }
}
